...
func QueueMarkupTimer(langData map[string]string, duration string, cplay string, videoid string, played string, dur string) *tb.ReplyMarkup {
	...
	btnQueued := createBtn(menu, langData["QU_B_1"], fmt.Sprintf("GetQueued %s|%s", cplay, videoid), "")
	btnClose := createBtn(menu, langData["CLOSE_BUTTON"], "close", "")
	...
	if duration == "Unknown" {
		...
		btnTimer := createBtn(menu, timerText, "GetTimer", "")
		menu.Inline(
			menu.Row(btnTimer),
...
func QueueBackMarkup(langData map[string]string, cplay string) *tb.ReplyMarkup {
	...
	btnBack := createBtn(menu, langData["BACK_BUTTON"], fmt.Sprintf("queue_back_timer %s", cplay), "")
	btnClose := createBtn(menu, langData["CLOSE_BUTTON"], "close", "")
...
func AQMarkup(langData map[string]string, chatID int64) *tb.ReplyMarkup {
	...
	menu.Inline(
		menu.Row(
			createBtn(menu, "▷", "ADMIN Resume|"+cid, ""),
			createBtn(menu, "II", "ADMIN Pause|"+cid, ""),
			createBtn(menu, "↻", "ADMIN Replay|"+cid, ""),
			createBtn(menu, "‣‣I", "ADMIN Skip|"+cid, ""),
			createBtn(menu, "▢", "ADMIN Stop|"+cid, ""),
		),
		menu.Row(
			createBtn(menu, langData["CLOSE_BUTTON"], "close", ""),
		),
	)
	return menu
}
...
func QueueAdvancedMarkup(langData map[string]string, vidid string, chatID int64, botUsername string) *tb.ReplyMarkup {
	...
	menu.Inline(
		menu.Row(createBtn(menu, langData["S_B_5"], "", fmt.Sprintf("https://t.me/%s?startgroup=true", botUsername))),
		menu.Row(
			createBtn(menu, "ᴘᴀᴜsᴇ", "ADMIN Pause|"+cid, ""),
			createBtn(menu, "sᴛᴏᴘ", "ADMIN Stop|"+cid, ""),
			createBtn(menu, "sᴋɪᴘ", "ADMIN Skip|"+cid, ""),
		),
		menu.Row(
			createBtn(menu, "ʀᴇsᴜᴍᴇ", "ADMIN Resume|"+cid, ""),
			createBtn(menu, "ʀᴇᴘʟᴀʏ", "ADMIN Replay|"+cid, ""),
		),
		menu.Row(createBtn(menu, "ᴍᴏʀᴇ", "", "https://t.me/betabot_hub")),
	)
	return menu
}
